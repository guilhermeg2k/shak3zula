...
function api.get_updates(timeout, offset, limit, allowed_updates, use_beta_endpoint) -- https://core.telegram.org/bots/api#getupdates
  ...
  local success, res = api.request(string.format('https://api.telegram.org/%sbot%s/getUpdates', use_beta_endpoint and 'beta/' or '', api.token), {
    ['timeout'] = timeout,
    ['offset'] = offset,
    ['limit'] = limit,
...
function api.send_reply(
  ...
  reply_parameters,
  reply_markup
) -- A variant of api.send_message(), optimised for sending a message as a reply.
...
function api.forward_messages(chat_id, from_chat_id, message_ids, message_thread_id, disable_notification, protect_content) -- https://core.telegram.org/bots/api#forwardmessages
...
function api.copy_messages(chat_id, from_chat_id, message_ids, message_thread_id, disable_notification, protect_content, remove_caption) -- https://core.telegram.org/bots/api#copymessages
  message_ids = type(message_ids) == 'table' and json.encode(message_ids) or message_ids
...
function api.edit_message_live_location(
  chat_id,
  message_id,
  inline_message_id,
  latitude,
...
function api.send_dice(chat_id, emoji, message_thread_id, disable_notification, protect_content, reply_parameters, reply_markup) -- https://core.telegram.org/bots/api#senddice
  reply_parameters = type(reply_parameters) == 'table' and json.encode(reply_parameters) or reply_parameters
  reply_markup = type(reply_markup) == 'table' and json.encode(reply_markup) or reply_markup
...
function api.promote_chat_member(
  chat_id,
  user_id,
  is_anonymous,
  can_manage_chat,
...
function api.edit_message_text(chat_id, message_id, text, parse_mode, entities, link_preview_options, reply_markup, inline_message_id) -- https://core.telegram.org/bots/api#editmessagetext
  entities = type(entities) == 'table' and json.encode(entities) or entities
  link_preview_options = type(link_preview_options) == 'table' and json.encode(link_preview_options) or link_preview_options
  reply_markup = type(reply_markup) == 'table' and json.encode(reply_markup) or reply_markup
  parse_mode = (type(parse_mode) == 'boolean' and parse_mode == true) and 'MarkdownV2' or parse_mode
...
function api.edit_message_caption(chat_id, message_id, caption, parse_mode, caption_entities, reply_markup, inline_message_id) -- https://core.telegram.org/bots/api#editmessagecaption
  caption_entities = type(caption_entities) == 'table' and json.encode(caption_entities) or caption_entities
  reply_markup = type(reply_markup) == 'table' and json.encode(reply_markup) or reply_markup
  parse_mode = (type(parse_mode) == 'boolean' and parse_mode == true) and 'MarkdownV2' or parse_mode
...
function api.send_sticker(chat_id, sticker, message_thread_id, emoji, disable_notification, protect_content, reply_parameters, reply_markup) -- https://core.telegram.org/bots/api#sendsticker
  reply_parameters = type(reply_parameters) == 'table' and json.encode(reply_parameters) or reply_parameters
  reply_markup = type(reply_markup) == 'table' and json.encode(reply_markup) or reply_markup
...
function api.create_invoice_link(
  title,
  description,
  payload,
  provider_token,
...
function api.send_game(chat_id, game_short_name, message_thread_id, disable_notification, protect_content, reply_parameters, reply_markup) -- https://core.telegram.org/bots/api#sendgame
  reply_parameters = type(reply_parameters) == 'table' and json.encode(reply_parameters) or reply_parameters
  reply_markup = type(reply_markup) == 'table' and json.encode(reply_markup) or reply_markup
...
function api.chat_administrator_rights(
  is_anonymous,
  can_manage_chat,
  can_delete_messages,
  can_manage_video_chats,
...
function api.reply_parameter(message_id, chat_id, allow_sending_without_reply, quote, quote_parse_mode, quote_entities, quote_position)
