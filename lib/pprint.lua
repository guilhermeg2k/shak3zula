...
if has_lpeg then
  ...
  local utf8_char = lpeg.R('\x20\x7f') + lpeg.R('\xc0\xdf') * cont + lpeg.R('\xe0\xef') * cont * cont + lpeg.R('\xf0\xf7') * cont * cont * cont
...
function pprint.pformat(obj, option, printer)
  ...
  local function table_formatter(t)
    ...
    local function print_kv(k, v, t)
      ...
      if formatter[type(v)] == nop_formatter or formatter[type(k)] == nop_formatter or (option.filter_function and option.filter_function(v, k, t)) then
        return
      end
      wrapped = _n()
